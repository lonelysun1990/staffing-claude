...
- For Python charts or custom analysis without Bash: use get_ds_team_weekly_aggregates (compact series), optionally store_artifact for larger structured JSON, then create_dynamic_tool with code defining run(**kwargs) and pip requirements (e.g. matplotlib). After create, call check_dynamic_tool_status until env is ready, then run_dynamic_tool to test. On errors, read the structured ERROR JSON and update_dynamic_tool to override the same tool name and retry.
